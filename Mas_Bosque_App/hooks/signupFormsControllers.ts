...
export function useEmailController() {
    ...
    const handleClick = () => {
        if (!emailRegex.test(signupData.email)) {
            ...
            Logger.warn(msg, {hasEmail: !!signupData.email});
...
export function useMedicalController() {
    ...
    const sanitizedBloodType = signupData.bloodType.trim().toUpperCase();
    const isValid = validBloodTypes.includes(sanitizedBloodType) &&
        signupData.allergies.trim().length <= 100 &&
        signupData.medicalConditions.trim().length <= 100 &&
        signupData.medications.trim().length <= 100;
...
export function useNameController() {
    ...
    const sanitizedName = signupData.name.trim();
    const sanitizedLastName = signupData.lastName.trim();
    const isValid =
        sanitizedName.length > 0 &&
        sanitizedName.length <= 50 &&
        sanitizedLastName.length > 0 &&
